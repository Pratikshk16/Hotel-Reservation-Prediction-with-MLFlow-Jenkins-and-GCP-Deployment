...
class DataIngestion:
    ...
    def download_csv_from_gcp(self):
        try:
            ...
            logger.error(f"Error while downloading CSV from GCP bucket {self.bucket_name}.")
    ...
    def split_data(self):
        try:
            ...
            train_data, test_data = train_test_split(data, test_size=self.config["test_ratio"], random_state=42)
    ...
    def run(self):
        try:
            ...
            logger.error(f"Custom exception occurred during data ingestion: {str(ce)}")
        finally:
